...
The 2026-08-14 `1249` QA1-to-PROD attempt found a separate Admin orchestration defect: the Targets detail header submitted the reusable Target before the customer route, reaching the legacy whole-store Target writer. The route remained QA1 and no external effect occurred, but Jobs recorded 24 write throttles. Do not retry this route change on an Admin version before the route-only hotfix. The safe contract is one focused selected-customer Output Route request, authoritative route readback, zero Target request, and zero Jobs/cache/mapping/other-customer/Proof write. Reusable Target editing must remain unavailable until its own focused persistence boundary is deployed.

For an authenticated Manual Import preview, distinguish request-local preparation from saved setup. Uploading or choosing workbook columns must not save anything. Preview creation may write one Job and one Pathfinder Order ID only after all source rows, existing approved product mappings, canonical fields, dates, Lift payload, route, and submit profile validate. It must not update the selected Import Method or ProductMappings. On a validation or mapping failure, reconcile both Jobs and OrderIds as unchanged before another attempt. The effective mappings and resolver settings belong on the resulting Job as immutable preparation evidence. Scheduled Wrike and public-intake paths are separate and must not be used as a preview workaround.

The Job and Pathfinder Order ID must be created in the same conditional DynamoDB transaction. A preview path that reserves the Order ID first or invokes `writeStore` / `writeDynamoStore` is unsafe and must be stopped. On a transaction conflict, confirm both tables are unchanged before a new attempt. On a timeout or otherwise uncertain response, do not retry: read the exact proposed Job and Order ID identities first, and resume only after their durable state is proven.
