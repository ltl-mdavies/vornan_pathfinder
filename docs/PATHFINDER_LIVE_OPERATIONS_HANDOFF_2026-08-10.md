...
Targets are reusable destination definitions. Output Routes are stored per customer by the composite customer/route identity. The same route ID therefore appears separately for Momentara `284619` and LTL Demo `1249`: Momentara remains on `env-lift-prod` with its active Live Customer path, while `1249` remains on `env-lift-qa1`. Scheduled intake is pinned to customer `284619` and Import Method `method-1784901795973`. Seeing QA1 while `1249` is selected does not change or expose Momentara, and no emergency route change is required.

The retained `1249` Manual XLSX method is still unused but stores a legacy Momentara product recipe. Admin commit `9076a7a1e1c07ba8ee673fa60591d301ff908ec7` is deployed with **Choose during preload** as the exact neutral source choice (`source_column`, prefix, and suffix blank; composite columns empty; fallback `none`; direct identifier `null`). Workbook-specific source selection remains local until an explicit mapping save. The same release adds concise **Reusable target** and **Customer route** labels with the selected customer name.

The first separately authorized attempt to change only `1249`'s customer route from QA1 to PROD exposed a remaining Admin orchestration defect. The Targets detail header invoked `PUT /api/targets/:id` before the focused customer Output Route endpoint. The legacy Target persistence path attempted a whole-store rewrite, advanced top-level monotonic metadata without changing the embedded route, and recorded 24 Jobs-table `WriteThrottleEvents` at `2026-08-14T22:07Z`; it failed before the focused route request. Authoritative data remains `1249` route `env-lift-qa1` and reusable Target active environment PROD. The `1249` workspace and Import Method are unchanged; mappings, Jobs, and Submit Attempts remain zero; no preview, provider, Lift, Wrike, or Proof action occurred.

Do not retry the QA1-to-PROD route save on the deployed Admin. The repository hotfix routes customer-only edits and the Primary Route Environment control directly through the focused customer Output Route endpoint, never through the shared Target endpoint, and uses fixed local success/failure copy. Reusable Target editing remains visibly unavailable until it has its own focused persistence boundary. The hotfix is Admin/test/docs only and must be separately reviewed and deployed before the `1249` configuration/mapping/preview sequence resumes.
