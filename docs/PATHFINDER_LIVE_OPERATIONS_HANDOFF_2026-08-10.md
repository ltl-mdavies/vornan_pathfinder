...
The customer-`1249` pilot also exposed a separate preview-side-effect boundary. The deployed authenticated Manual Import preview used request-local mapping and resolver choices to prepare the order, but also persisted the selected Import Method and observed ProductMappings before returning. The bounded repository fix makes the direct authenticated preview explicitly request-local: it retains the effective field mappings, product resolver, order-name resolver, and Ext_ID strategy on the Job, while leaving the saved method and mapping table unchanged. It performs existing-mapping resolution plus canonical, date-format, Lift-payload, route, and submit validation before reserving a Pathfinder Order ID. A missing mapping or invalid date writes neither Job nor Order ID. Scheduled Wrike and public-intake preparation retain their existing service behavior. This contract is not live until its exact API artifact is deployed and reconciled; do not generate the `1249` preview beforehand.

The first post-deployment preview action was stopped before invocation after a final DynamoDB review found that the successful request-local path still reserved its Order ID before the Job write. The follow-up repository hotfix generates the candidate identity without persisting it, then conditionally creates the exact Order ID and exact Job in one DynamoDB transaction. Its least-privilege API-role update adds `TransactWriteItems` only to those two table ARNs in addition to the existing workspace/mapping transaction resources. It cannot update the selected workspace, method, route, mappings, cache, another Job, attempts, status records, or Proof data. A transaction conflict writes neither record; an unconfirmed persistence response returns explicit refresh-and-reconcile guidance and is never retried automatically. Keep the `1249` preview and submit stopped until this additional hotfix is merged, API-deployed through an inspected IAM/code-only change set, reconciled, and receives a fresh exact preview action lock.
