...
The deployed single-mapping endpoint still reaches `writeStore`, which rewrites unrelated workspace mirrors and Jobs. Treat the response as unconfirmed even though records may have been retained. Do not click **Approve** again, delete duplicates, replace the mapping list, create the pilot preview, or continue to Lift/Proof until the six exact records are reconciled and a separate action is authorized.

The bounded repository fix uses one conditional ProductMappings-table transaction for the exact customer-route/version and mapping ID. Identical approval is idempotent; failure cannot fall through to another table. The API provides sanitized `success`, `conflict`, `invalid`, or `persistence_uncertain` telemetry with `external_effects=false`, and Admin reports saving/saved/error beside the affected row. This release requires no migration and must not modify the six retained mappings during deployment. Deploy API first, verify zero Jobs writes/throttles and a natural Momentara cycle, then deploy Admin. Any mapping recovery or preview remains a later exact action.

### 2026-08-14 Release A and route-scope clarification

Release A is deployed from application commit `6a6d436ff30c570b0d6a0ecdb32f75a263855145`. Its focused persistence boundaries, neutral new-customer construction, and product-workbook setup improvements required no migration or backfill. Post-release protected counts were Customers 2, CustomerWorkspaces 2, Targets 2, ImportMethods 3, OutputRoutes 2, ProductMappings 282, Jobs 60, OrderIds 63, SubmitAttempts 23, LiftProductCache 353, OrderStatusTokens 24, OrderStatusSnapshots 16, CanonicalRegistry 1, ProofCore 142, and ProofAudit 147. A natural post-API scheduler cycle replayed all ten candidates with zero preparation, Lift submit, Wrike write, failure, or candidate failure; all Pathfinder and Proof alarms were `OK` and all queues were empty.

Targets are reusable destination definitions. Output Routes are stored per customer by the composite customer/route identity. The same route ID therefore appears separately for Momentara `284619` and LTL Demo `1249`. Both customer route records now use `env-lift-prod`; they remain independent. Scheduled intake stays pinned to customer `284619` and Import Method `method-1784901795973`.

Admin commit `9076a7a1e1c07ba8ee673fa60591d301ff908ec7` added **Choose during preload** as the exact neutral source choice. Route-only Admin hotfix `cd221897e50c03d91afea41b068eb6000db6b4ac` then removed the shared-Target save from customer-route changes. One authorized action changed only `1249`'s route from QA1 to PROD, with no Momentara, shared-Target, mapping, Job, attempt, cache, provider, Lift, Wrike, or Proof effect.

At `2026-08-14T23:28:30.632Z`, a separately authorized focused Import Method save stored the neutral `1249` product-resolution contract: blank source/prefix/suffix, empty composites, `derived_key`, `map_to_lift_unit`, fallback `none`, and no direct-unit column. Telemetry recorded only workspace and Import Method persistence with `external_effects=false`; protected counts and all operational gates remained unchanged.

The save also changed `workspace.templates[template_manual_xlsx_v1].status` from `Draft` to `Published` because the deployed store mirrors the already-Active method status. Public intake remains disabled, with no public key or publication timestamp, so no external capability was enabled. Nevertheless, the transition was outside the approved action boundary. Do not run the catalog-`6338` refresh or resume the Proof pilot yet. The repository hotfix preserves every existing linked template field exactly during ordinary Import Method saves and creates a missing legacy mirror only as `Draft`. It has no migration/backfill and does not automatically rewrite the retained `1249` template; any correction requires separate authorization.

The earlier failed QA1-to-PROD attempt remains retained incident evidence: the legacy Targets header path invoked `PUT /api/targets/:id`, attempted a whole-store rewrite, and recorded 24 Jobs-table `WriteThrottleEvents` at `2026-08-14T22:07Z` before the route request. The deployed route-only hotfix prevents that path for customer-route changes. Reusable Target editing remains visibly unavailable until it receives its own focused persistence boundary.

### 2026-08-15 template-preservation release and PROD catalog safety hold

API commit `e331f7531c5b56eb739fdca386d8a66a81ac9e56` is deployed. Workflow `31854780059` / job `94937352940` produced artifact `api/pathfinder-api-lambda-e331f7531c5b56eb739fdca386d8a66a81ac9e56.zip`, S3 version `BvR5xDQ5sK8Vjt4c.0sc_EkR7vM0ldG6`, ETag `e68b31f4c35a85b3368464bdb3c0cd4c`, Lambda SHA-256 `+H82W0rZTqTGigPE8VcF3pu6p6E9t2BHr3p69k3D1I0=`, revision `784b8f22-2a0c-4f54-a5e1-e0ebb5582074`. It preserves an existing linked workspace template exactly during ordinary Import Method saves and creates a missing mirror only as `Draft`. It does not migrate or correct the retained `1249` Published template.

The stack returned to `UPDATE_COMPLETE`; health was HTTP 200; all non-code parameters were preserved. Protected counts remained Customers 2, CustomerWorkspaces 2, Targets 2, ImportMethods 3, OutputRoutes 2, ProductMappings 282, Jobs 60, OrderIds 63, SubmitAttempts 23, LiftProductCache 353, OrderStatusTokens 24, OrderStatusSnapshots 16, CanonicalRegistry 1, ProofCore 142, and ProofAudit 147. Natural cycle `wrike_scheduled_20260815005753101`, checked `2026-08-15T00:57:53.101Z`, replayed ten candidates with zero preparations, Lift submissions, Wrike writes, or failures. Alarms were `OK`, queues empty, and the disabled Proof scan worker remained unmapped.

The next bounded action was one customer-`1249` / PROD / company-`91` / catalog-`6338` refresh. Read-only preflight found the cache at 353 rows: QA1 catalog `8102=106`, QA1 catalog `6338=16`, PROD catalog `8102=228`, and PROD catalog `6338=0`. The action was stopped before any provider request or mutation because cache identity excludes catalog ID. A returned catalog-`6338` product whose normalized product identity already exists in PROD catalog `8102` would overwrite the retained row. The collision set cannot be proven before the provider response, while the deployed request fetches and persists together.

The repository guard in review checks every normalized incoming key against the exact existing target/environment/company/product key with strongly consistent reads before the first write. It also stores a durable catalog-scope marker and uses transactional write-time conditions, closing the race between preflight and persistence: an intervening different-catalog owner makes the affected transaction fail without overwriting that row. A known different-catalog identity returns `catalog_identity_collision` / `lift_catalog_cross_catalog_identity_collision`, reports only bounded collision and fetched counts, and writes nothing; a concurrent conflict reports any earlier definitely persisted batch rather than claiming a false zero. Same-catalog refresh remains idempotent, QA1 and PROD remain isolated, and a collision-free response retains the additive cache-only writer and the existing partial/uncertain reconciliation contract. Do not retry the PROD refresh until this guard is merged, deployed, and given a fresh action lock. A later action must still perform exactly one provider GET, stop without retry on any collision/partial/uncertain result, and prove zero Jobs, mappings, attempts, configuration, Momentara, Wrike, Lift-order, or Proof effects.

The customer-`1249` pilot also exposed a separate preview-side-effect boundary. The deployed direct authenticated preview is now explicitly request-local: it retains effective field mappings, product resolver, order-name resolver, and Ext_ID strategy on the Job while leaving the saved Import Method and ProductMappings unchanged. Existing-mapping resolution plus canonical, date-format, Lift-payload, route, and submit validation runs before durable identity creation. Missing mappings, missing uploaded identity evidence, or invalid dates write neither Job nor Order ID. Scheduled Wrike and public-intake preparation retain their existing service behavior.

API `8fb293d4458fdfdd1c015849f81d88cb660284de` generates the candidate identity without persisting it, then conditionally creates the exact Order ID and exact Job in one DynamoDB transaction. Its least-privilege role allows `TransactWriteItems` only for those two tables alongside the existing focused workspace/mapping boundary. It cannot update the selected workspace, method, route, mappings, cache, another Job, attempts, status records, or Proof data. A transaction conflict writes neither record; an unconfirmed persistence response requires reconciliation and is never retried automatically. Admin `0be1ca144d9c18669a62048d8711e6e198294408` is deployed with the uploaded-workbook product and identity choices.

The retained earlier `1249` preview is job `job_20260815124334_f8bcbe`, Ext_ID `PFMSUDA9EOD259`, state **Ready**. One supervised Lift submit attempt returned a deterministic HTTP 400 because the LTL Demo customer lacked the High End Work order-type mapping. No target Lift order or Proof action exists; do not retry. Keep that Lift-side HOLD separate from the 2026-08-17 mapping-approval incident. The later three-product workflow has no new preview or external effect and remains stopped pending exact mapping reconciliation and authorization.
