...
The retained `1249` Manual XLSX method is still unused but stores a legacy Momentara product recipe. A repository-only Admin hotfix is pending review to expose **Choose during preload** as the exact neutral source choice (`source_column`, prefix, and suffix blank; composite columns empty; fallback `none`; direct identifier `null`). Workbook-specific source selection remains local until an explicit mapping save. The same hotfix adds concise **Reusable target** and **Customer route** labels with the selected customer name. It changes no API, storage, infrastructure, configuration, mapping, job, cache, provider, Lift, Wrike, or Proof path. Do not resume the `1249` configuration/mapping/preview sequence until this Admin-only hotfix is separately merged, deployed, and reconciled.
