...
- Keep `PATHFINDER_PROOF_ENABLE_PUBLIC_READ=false`, authenticated grant creation off, and `PATHFINDER_PROOF_ENABLE_LINK_EMAIL=false` for the first deployment.
...
3. Deploy the dedicated Proof stack with public read off, grant creation off, managed WAF on, and the edge secret configured. Record both `ProofCoreTable*` and `ProofAuditTable*` outputs for the authenticated Pathfinder API deployment.
...
8. With authenticated grant creation still off, call the grant route for an uncached QA order and confirm it returns disabled without a cache lookup, Lift request, grant, or raw link. Temporarily enable grant creation in the admin boundary only. Create a labeled `view` grant directly for one uncached QA order and confirm the route completes its first Lift `GET` synchronization before issuing the grant. Repeat with a fresh cached order and confirm no extra Lift read; repeat with a stale cached order and confirm refresh precedes issuance. Force a safe read failure and confirm no grant or raw link is produced. Capture the successful fragment link through the approved private channel; do not retain the raw link in evidence. With `PATHFINDER_PROOF_ENABLE_LINK_EMAIL=true` and `PATHFINDER_STATUS_EMAIL_MODE=log`, exercise the operator send control and confirm the raw link remains available because no message was sent. Confirm logs expose only the masked recipient and the audit stores only delivery mode/status.
9. With public read still off, confirm token exchange returns the unavailable response without revealing order existence.
10. After review, record the isolated read-only lifecycle as confirmed and deploy with public read on. Repeat the automated smoke gate.
