...
Targets remain reusable destination definitions. Output Routes are selected-customer records, keyed by customer and route ID. Production reads confirm Momentara `284619` and LTL Demo `1249` now each use their own `env-lift-prod` route record; the shared route ID does not merge those records.

Admin commit `9076a7a1e1c07ba8ee673fa60591d301ff908ec7` added an explicit **Choose during preload** source option. The later route-only hotfix at `cd221897e50c03d91afea41b068eb6000db6b4ac` removed the unsafe shared-Target save from customer-route changes. One authorized route-only action then changed only customer `1249` from QA1 to PROD; Momentara, the shared Target, mappings, Jobs, attempts, cache, and Proof state were unchanged, with no throttles or external effect.
...
The cause was the legacy whole-store Dynamo writer: after the four setup records were durable, it attempted an unbounded `PutItem` rewrite of every existing Job and a replacement pass over the Lift product cache. Release A removed that writer from workspace creation, Import Method, Output Route, catalog-preset, and catalog-cache persistence.

Subsequent authenticated attempts to refresh Lift catalog `6338` exposed the same legacy path and produced 28, 24, and 85 Jobs-table write throttles without changing the cache. After Release A, one separately authorized QA1 refresh persisted 16 normalized catalog-`6338` rows through the focused additive cache boundary with no Jobs write or throttle. Current cache is 353 total / catalog `8102` 334 / catalog `6338` 16. Customer `1249` still has zero mappings, Jobs, and Submit Attempts. A PROD refresh remains separately gated and is currently stopped by the template-lifecycle incident above.

The deployed focused boundaries use customer-scoped conditional transactions. Lift catalog refresh additively upserts only normalized rows keyed by target/environment/company/product identity; it cannot scan, delete, replace existing cache rows, or write Jobs, mappings, workspace configuration, attempts, or another customer. Completed cache batches are counted exactly, while partial or uncertain outcomes reconcile only exact intended keys and use sanitized operator copy and telemetry. New customer setup is explicit and idempotent; workspace reads are non-writing.
