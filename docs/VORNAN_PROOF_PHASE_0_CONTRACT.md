...
Phase 2 is implemented but not deployed or enabled. `scripts/deploy-proof-stack.sh` defaults to the `dev` environment and keeps public read off; `scripts/deploy-proof-web.sh` publishes only the static SPA. Follow `docs/VORNAN_PROOF_READ_ONLY_QA_RUNBOOK.md`. Deployment must keep public read and grant creation off until the dedicated table, isolated same-origin routing, secure-cookie exchange, logical grant expiry, session TTL, revocation, one-order isolation, background refresh, WAF attachment, alarms, and sanitized telemetry are confirmed in the target environment. This gate does not change the Lift write prohibition.
