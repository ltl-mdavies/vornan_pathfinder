...
- The SPA revalidates asset URLs before rendering. PNG/JPEG/GIF/WebP use contained images; PDF is rendered into a contained canvas by the Vornan Proof viewer rather than an embedded browser PDF surface. PDF and image creatives share the same movable, keyboard-repositionable Vornan control island; PDFs add page navigation, fit-page/fit-width/actual-size modes, and rotation while images retain fit and zoom controls. Explicit open/download actions remain available outside the viewer. TIFF/PSD/AI/EPS/INDD and other non-browser-native files use a metadata card and safe full-resolution action without an embedded active preview. A valid 676-byte one-page PDF fixture and a development-only `#/proof/assets-qa` packet cover PDF, long filename, prepress download-only, conversion-unavailable states, and the unified control surface.
...
The next read-only activation review is bounded by `scripts/proof-readonly-activation-review.mjs` and `docs/VORNAN_PROOF_READ_ONLY_ACTIVATION_REVIEW_PACKET_2026-07-21.md`. It requires exact dev/cohort/time scope, private link handoff, monitoring and rollback ownership, support escalation, and intact no-production-approval/no-DNS/no-email/no-decision/no-Lift-write constraints. The LTL Demo cohort is enforced by internal Lift customer ID at authenticated grant creation, and the UTC activation deadline caps grants/sessions and fails closed. The checker is non-mutating and hard-codes every change/deployment authorization false, including when all prerequisites and explicit approval are recorded.
