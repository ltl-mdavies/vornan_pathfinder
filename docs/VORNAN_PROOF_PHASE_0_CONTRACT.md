...
Phase 2 is implemented and its isolated `vornan-proof-dev` foundation is deployed dark, but no customer capability is enabled. Dark-boundary, purgeable synthetic-lifecycle, direct-Lift read-only, and Pathfinder-originated read-only evidence are recorded in the dated Proof QA documents. `npm run check:proof-phase2` evaluates their bounded readiness state without mutating infrastructure or authorizing a feature-flag change. The current result recognizes the isolated read evidence as complete while blocking activation until the deployed grant/session lifecycle, deployed one-order customer boundary, and explicit activation approval are separately recorded. `scripts/deploy-proof-stack.sh` defaults to the `dev` environment and keeps public read off; `scripts/deploy-proof-web.sh` publishes only the static SPA. Follow `docs/VORNAN_PROOF_READ_ONLY_QA_RUNBOOK.md`. Deployment must keep public read and grant creation off until the dedicated table, isolated same-origin routing, secure-cookie exchange, logical grant expiry, session TTL, revocation, one-order isolation, background refresh, WAF attachment, alarms, and sanitized telemetry are confirmed in the target environment. This gate does not change the Lift write prohibition.

The next customer-boundary validation is prepared in `scripts/proof-customer-boundary-qa.ts` and `docs/VORNAN_PROOF_CUSTOMER_BOUNDARY_QA_PLAN_2026-07-21.md`, but it is not authorized or executed. The runner accepts only the alias-free `vornan-proof-dev` public-read QA posture, the retained reserved synthetic fixture, and an exact confirmation string. It exercises no email, decision, production-public-read approval, or Lift transport and always revokes its temporary view grant on exit. Its preflight continues to report deployment and mutation authorization as false; a separate explicit approval is required before the temporary dev stack parameter changes.
